...
namespace _20250313_focistak_dani_hf
{
    ...
    internal class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            // 5.feladat
            Console.WriteLine("\n");
            Dictionary<string, Jatekos> orszagGoljai = new Dictionary<string, Jatekos>();
            foreach (Jatekos jatekos in jatekosLista)
            {
                if(!orszagGoljai.ContainsKey(jatekos.CsapatanakOrszaga)) 
                {
                    orszagGoljai.Add(jatekos.CsapatanakOrszaga, jatekos);
                } else
                {
                    if (orszagGoljai[jatekos.CsapatanakOrszaga].GolokAzEvben < jatekos.GolokAzEvben)
                    {
                        orszagGoljai[jatekos.CsapatanakOrszaga] = jatekos;
                    }
                }
            }
            foreach (var orszag in orszagGoljai)
            {
                Console.WriteLine($"{orszag.Key} országban a legtöbb gól: {orszag.Value.Nev}");   
            }
            ...
            for (int i = 1; i < jatekosLista.Count; i++)
            {
                ...
                if(Math.Abs(jatekos.GolokAzElozoEvben - jatekos.GolokAzEvben) < Math.Abs(legkisebbKulJatekos.GolokAzElozoEvben - legkisebbKulJatekos.GolokAzEvben))
                {
            ...
            else
            {
                for (int i = 0; i < 3 && i < jatekosokAzOrszagbol.Length; i++)
                {
                    ...
                    Console.WriteLine($"Gólok előző évben: {jatekos.GolokAzElozoEvben} \t Gólok ebben az évben: {jatekos.GolokAzEvben} \t Ország: {jatekos.CsapatanakOrszaga}\n");
                }
            }
